...
from core.utils import get_error_http_response_object_not_found, call_bitmex_api, get_http_response_for_key_error, \
...
def get_chart_data(request):
    ...
    if chart_name == "balance":
        ...
        balance_history = call_bitmex_api('/user/walletHistory', api_key=user.key_pub, api_secret=user.key_secret)
        profit_history = []
        ignore_transact_type_list = ["Deposit", "Withdrawal"]
        profit = 0
        profit_history.append({"timestamp": balance_history[-1]["timestamp"], "value": 0})

        for bh in reversed(balance_history):
            if bh["transactType"] not in ignore_transact_type_list:
                profit += bh["amount"]

                now = timezone.now()
                timestamp = parse(bh["timestamp"]) if bh["timestamp"] is not None else now
                if timestamp > now:
                    timestamp = now

                profit_history.append({"timestamp": timestamp.strftime('%Y-%m-%dT%H:%M:%S.000Z'), "value": profit})
